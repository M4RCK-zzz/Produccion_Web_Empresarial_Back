...
def _obtener_graficos(db: Session) -> dict:
    ...
    for i in range(5, -1, -1):
        ...
        # Conteo seguro de comentarios por mes
        total_comentarios = (
        ...
        # Conteo seguro de comentarios positivos
        positivos = (
            db.query(ComentarioModel)
    ...
    # Distribución por sentimiento con valores por defecto
    total_analisis = db.query(AnalisisNlpModel).count() or 0
    pos_total = db.query(AnalisisNlpModel).filter(AnalisisNlpModel.confianza > 0.05).count() or 0
    neg_total = db.query(AnalisisNlpModel).filter(AnalisisNlpModel.confianza < -0.05).count() or 0
    neu_total = max(total_analisis - pos_total - neg_total, 0)

    # Tiempo promedio de atención sin fallos por NoneType
...
def obtener_graficos_dashboard(db: Session = Depends(get_db)):
    ...
    try:
        ...
        print(f"Error interno en /api/dashboard/graficos: {e}")
        raise HTTPException(
